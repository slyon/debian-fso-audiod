...
gint _vala_main (gchar** args, int args_length1) {
	...
	FsoFrameworkDBusSubsystem* _tmp22_;
	guint _tmp23_ = 0U;
	guint count;
	...
	GError * _inner_error_ = NULL;
	{
		...
		opt_context = _tmp0_;
		_tmp1_ = opt_context;
		g_option_context_set_summary (_tmp1_, "FreeSmartphone.org Audio daemon");
		_tmp2_ = opt_context;
		g_option_context_set_description (_tmp2_, "This daemon implements the freesmartphone.org Audio API");
		_tmp3_ = opt_context;
	...
	bus_type = _tmp18_;
	_tmp19_ = bus_type;
	_tmp20_ = fso_framework_dbus_subsystem_new ("fsoaudio", _tmp19_);
	subsystem = _tmp20_;
	_tmp21_ = subsystem;
	fso_framework_subsystem_registerPlugins ((FsoFrameworkSubsystem*) _tmp21_);
	_tmp22_ = subsystem;
	_tmp23_ = fso_framework_subsystem_loadPlugins ((FsoFrameworkSubsystem*) _tmp22_);
	count = _tmp23_;
	...
	if (_tmp28_ > ((guint) 0)) {
		...
		fso_framework_logger_info (_tmp30_, "fsoaudiod => mainloop");
		...
		fso_framework_logger_info (_tmp32_, "mainloop => fsoaudiod");
	}
	_tmp33_ = fso_framework_theLogger;
	fso_framework_logger_info (_tmp33_, "fsoaudiod shutdown.");
	result = 0;
